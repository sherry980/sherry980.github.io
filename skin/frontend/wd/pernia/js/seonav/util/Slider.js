...
S.manager = {
    ...
    createAll : function() {
        ...
        for (i=sliderNodes.length - 1; i >= 0 ; i--) { // loop backwards, as input node may be removed during slider creation
            var newSlider = new S( sliderNodes[i].id ?  sliderNodes[i].id : idCounter++, sliderNodes[i]);
    ...
    destroyAll : function() {
        var key, instance, container;

        if (S.instances) {
            for (key in S.instances._items) {
                instance = S.instances._items[key];
                instance.removeHandlers();
                container = $(instance.id);
                if (container) {
                    Element.select(container, 'input.tpgSliderInput').each(function (e) {
                        Element.insert(container, e);
                    });
                    Element.select(container, 'div').each(function (e) {
                        Element.remove(e);
                    });
                }
            }
        ...
        if (DoubleSlider.instances) {
            for (key in DoubleSlider.instances._items) {
                instance = DoubleSlider.instances._items[key];
                instance.removeHandlers();
                container = $(instance.id);
                if (container) {
                    Element.select(container, 'input.tpgSliderInput').each(function (e) {
...
TPG.Event.addHandler(window, 'resize', function(){
