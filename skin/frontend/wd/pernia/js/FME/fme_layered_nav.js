...
function fme_layered_show_products(transport)
{
    ...
    if (resp.products) {
		...
		//$('current_url_pagn').value = $('fme_layered_ajax').value + '?' + $('fme_layered_params').value;
//         ias.destroy(); 
//         ias.bind();
...
//        	next: '.next',
//        	loader: '<img src="/skin/frontend/default/mytheme/images/ajaxscroll/loader.gif"/>',
//        	
...
//         jQuery('img.lazy').jail();
	...
	//show selected filters
		jQuery("dd.scrollbar a").each(function(){
			if(jQuery(this).hasClass("fme_layered_attribute_selected")){
				var gteid= jQuery(this).attr("id");
				var getclass = gteid.split("-",1);
				var getid= '#fme_layered_'+getclass[0];
				jQuery(getid).show();
			}
		});
}
